...
def get_num_key_value_heads(model_params):
    # Qwen models use grouped-query attention (GQA)
    # For example, Qwen2.5-0.5B has 14 query heads but only 2 key-value heads
    if hasattr(model_params, "num_key_value_heads"):
        return getattr(model_params, "num_key_value_heads")
    # Default fallback if not specified
    return getattr(model_params, "num_attention_heads") // 7  # Approximation based on Qwen2.5-0.5B ratio
...
def get_intermediate_size(model_params):
    # Qwen models may use intermediate_size or ffn_dim as the parameter name
    if hasattr(model_params, "intermediate_size"):
        return getattr(model_params, "intermediate_size")
    elif hasattr(model_params, "ffn_dim"):
        return getattr(model_params, "ffn_dim")
    else:
        # Default approximation based on common transformer architectures
